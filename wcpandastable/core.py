...
class Table(Canvas):
    ...
    # [NG] added absolute_path to application py file and callback function
    def __init__(self, parent=None, model=None, dataframe=None,
                 callback=None, width=None, height=None,
        ...
        # [NG] added parent callback function
        self.parent_callback = callback
        #get platform into a variable
        print('\npandastable core reviewed')
        # [NG] added absolute_path class variable
        self.absolute_path = absolute_path
        # [NG] added db_filename class variable
        self.db_filename = os.path.join(self.absolute_path, 'warchest.db')
        ...
        self.childrow = 5
        self.loadPrefs()
        # [NG] self.currentdir changed from os.path.expanduser('~') to absolute_path
        self.currentdir = absolute_path
        ...
        # [NG] added self.currentcol_name
        self.currentcol_name = self.model.df.columns[self.currentcol]
        return
    ...
    # [NG] added execute_db_query method
    ...
    # [NG] added write_integer_to_db method
    ...
    # [NG] added write_text_to_db method
    ...
    # [NG] added is_castable method
    def is_castable(self, series, type_to_cast):
    ...
    def set_defaults(self):
        ...
        # [NG] self.cellbackgr changed from '#F4F4F3' to '#FFFFFF' (255,255,255)
        self.cellbackgr = '#FFFFFF'
        self.entrybackgr = 'white'
        # [NG] self.grid_color changed from '#ABB1AD' to '#D4D4D4' (212,212,212)
        self.grid_color = '#D4D4D4'
        # [NG] self.rowselectedcolor changed from '#E4DED4' to '#D2D2D2' (210,210,210)
        self.rowselectedcolor = '#D2D2D2'
        self.multipleselectioncolor = '#E0F2F7'
        # [NG] self.boxoutlinecolor changed from '#084B8A' to '#217346' (33,115,70)
        self.boxoutlinecolor = '#217346'
        # [NG] self.colselectedcolor changed from '#e4e3e4' to '#D2D2D2' (210,210,210)
        self.colselectedcolor = '#D2D2D2'
    ...
    def setColumnType(self):
        ...
        # [NG] modified type options
        # coltypes = ['object','str','int','float64','category']
        coltypes = ['object','str','int8','int16','int32','int64','uint8','uint16','uint32','uint64','float16','float32','float64','category','bool']
        curr = df[col].dtype
        ...
        if d.result == None:
            return
        self.storeCurrent() # [NG] Added
        t = d.results[0]
        # [NG] added a check to see if column can be casted safely
        # [NG] without changing underlying data
        dtype = df.dtypes[col]
        if (dtype == "float64" or dtype == "float32" or dtype == "float16" or
                dtype == "int" or dtype == "int8" or dtype == "int16" or
                dtype == "int32" or dtype == "int64" or dtype == "uint8" or
                dtype == "uint16" or dtype == "uint32" or dtype == "uint64" or
                dtype == "float"):
            if self.is_castable(df[col], t) is True:
                try:
                    self.model.df[col] = df[col].astype(t)
                    self.redraw()
                    # [NG] add transformation to log
                    trans = Transformation()
                    trans.add_transformation(session=self.get_session(),
                                             index=self.currentcol,
                                             name=col,
                                             transformation=sys._getframe().f_code.co_name,
                                             option1=t)
                except:
                    print('failed')
            else:
    ...
    def cleanData(self):
        ...
        df = self.model.df
        cols = df.columns
        # [NG] added more fill options: mean, median, mode
        # fillopts = ['', 'fill scalar', 'ffill', 'bfill', 'interpolate']
        fillopts = ['', 'fill scalar', 'ffill', 'bfill', 'interpolate', 'mean',
                    'median', 'mode']
        ...
        if method == '':
            ...
            df = df.interpolate()
        elif method == 'mean':  # [NG] added more fill options: mean
            df = df.fillna(df.mean())
        elif method == 'median':  # [NG] added more fill options: median
            df = df.fillna(df.median())
        elif method == 'mode':  # [NG] added more fill options: mode
    ...
    def createCategorical(self):
        ...
        df = self.model.df
        col = df.columns[self.currentcol]
        is_new_col_created = False  # [NG] added

        # [NG] added default value col to get_dummies prefix
        # [NG] added encode class labels
        # [NG] added map ordinal features
        d = MultipleValDialog(title='Categorical data',
                                initialvalues=(0,'',0,col,'','',0,'encoded_'+col,0,'ordinal_'+col),
                                labels=('Convert to integer codes:','Name:',
                                        'Get dummies:','Dummies prefix:',
                                        'Numerical bins:','Labels:',
                                        'Encode class labels:','Name:',
                                        'Map ordinal features:','Name:'),
                                ...
                                tooltips=(None, 'name if new column',
                                         ...
                                         'name if new column','will use active ordinal mapping',
                                         'name if new column'),
                                parent = self.parentframe)
        if d.result == None:
            return
        ...
        encode = d.results[6]  # [NG] added
        name_encode = d.results[7]  # [NG] added
        ordinal = d.results[8]  # [NG] added
        name_ordinal = d.results[9]  # [NG] added

        if name == '':
            name = col
        else:
            is_new_col_created = True
        if name_encode == '':  # [NG] added
            name_encode = col
        if name_ordinal == '':  # [NG] added
            name_ordinal = col
        if prefix == '':
            prefix = None
        if dummies == 1:
            ...
            if is_new_col_created:
                ...
                trans.add_transformation(session=self.get_session(),
                                         ...
                                         option2='',
                                         option3='')
        elif encode == 1:  # [NG] added
            class_mapping = {label: idx for idx, label
                             in enumerate(np.unique(df[col]))}
            df[name_encode] = df[col].map(class_mapping)
            # [NG] add transformation to log
            trans = Transformation()
            trans.add_transformation(session=self.get_session(),
                                     index=self.currentcol,
                                     ...
                                     transformation=sys._getframe().f_code.co_name,
                                     options="encode")
        elif ordinal == 1:  # [NG] added
            col_isactive = 'IsActive'
            query = 'SELECT OrdinalMapping FROM OrdinalMaps WHERE IsActive=?'.\
                format(col1=col_isactive)
            parameters = ("Yes",)
            cursor = self.execute_db_query(query, parameters)
            for row in cursor:
                ordinal_mapping = row[0]
            df[name_ordinal] = df[col].map(json.loads(ordinal_mapping))
            # [NG] add transformation to log
            trans = Transformation()
            trans.add_transformation(session=self.get_session(),
                                     index=self.currentcol,
    ...
    def handle_left_click(self, event):
        ...
        # [NG] save selected column to db
#        self.write_integer_to_db(table_name='TopAreaItems',
#                                 item_name='selected_column',
#                                 item_value=self.currentcol)
#        self.write_text_to_db(table_name='TopAreaItems',
#                              item_name='selected_column_name',
#                              item_value=self.model.df.columns[self.currentcol])
        self.parent_callback(self.currentcol,
                             self.model.df.columns[self.currentcol],
                             'describe_column')
        return
    ...
    def drawSelectedRect(self, row, col, color=None):
        ...
        if color == None:
            # [NG] color changed from 'gray25' to '#217346' (255,255,255)
            color = '#217346'
        w=2
        x1,y1,x2,y2 = self.getCellCoords(row,col)
