...
class ColumnHeader(Canvas):
    ...
    # [NG] bg changed from 'gray25' to #E6E6E6' (230,230,230)
    def __init__(self, parent=None, table=None):
        Canvas.__init__(self, parent, bg='#E6E6E6', width=500, height=40)
    ...
    def setDefaults(self):
        # [NG] self.colselectedcolor changed from '#0099CC' to '#D3F0E0' (211,240,224)
        self.colselectedcolor = '#D3F0E0'
        return
    ...
    def redraw(self):
        ...
        for col in self.table.visiblecols:
            ...
            # [NG] fill changed from 'white' to '#D4D4D4' (212,212,212)
            line = self.create_line(x, 0, x, h, tag=('gridline', 'vertline'),
                                 fill='#D4D4D4', width=1)
            # [NG] fill changed from 'white' to 'black' (0,0,0)
    ...
    def handle_left_click(self,event):
        ...
        self.draggedcol = None
        # [NG] color changed from 'lightblue' to '#9fd5b7' (159,213,183)
        self.drawRect(self.table.currentcol, tag='dragrect',
                      color='#9fd5b7', outline='white')
        ...
        self.table.write_integer_to_db(table_name='TopAreaItems',
                                       item_name='selected_column',
                                       item_value=self.table.currentcol)
        self.table.write_text_to_db(table_name='TopAreaItems',
                                    item_name='selected_column_name',
                                    item_value=self.model.
                                    df.columns[self.table.currentcol])
        return
    ...
    def draw_resize_symbol(self, col):
        ...
        x1,y1,x2,y2 = self.table.getCellCoords(0,col)
        # [NG] fill changed from 'white' to 'black' (0,0,0)
        # [NG] outline changed from 'gray' to 'white' (255,255,255)
        self.create_polygon(x2-3,h/4, x2-10,h/2, x2-3,h*3/4, tag='resizesymbol',
            fill='black', outline='white', width=wdth)
        # [NG] fill changed from 'white' to 'black' (0,0,0)
        # [NG] outline changed from 'gray' to 'white' (255,255,255)
    ...
    def drawRect(self,col, tag=None, color=None, outline=None, delete=1):
        ...
        if outline == None:
            # [NG] outline changed from 'gray25' to '#E6E6E6' (230,230,230)
            outline = '#E6E6E6'
        if delete == 1:
            self.delete(tag)
        w=1
...
class RowHeader(Canvas):
    ...
    # [NG] bg changed from 'gray75' to '#FFFFFF' (255,255,255)
    def __init__(self, parent=None, table=None, width=50):
        Canvas.__init__(self, parent, bg='white', width=width, height=None)
        if table != None:
            self.table = table
            self.width = width
            self.inset = 1
            # [NG] bg changed from '#C8C8C8' to '#E6E6E6' (230,230,230)
            self.color = '#E6E6E6'
    ...
    def redraw(self, align='w', showkeys=False):
        ...
        for col in cols:
            ...
            for row in col:
                ...
                # [NG] outline changed from 'white' to '#D4D4D4' (212,212,212)
                self.create_rectangle(x,y1,w-1,y2, fill=self.color,
                                        outline='#D4D4D4', width=1,
                                        tag='rowheader')
    ...
    def drawRect(self, row=None, tag=None, color=None, outline=None, delete=1):
        ...
        if tag==None:
            tag='rect'
        if color==None:
            # [NG] color changed from '#0099CC' to '#D3F0E0' (211,240,224)
            color='#D3F0E0'
        if outline==None:
            outline='gray25'
        if delete == 1:
